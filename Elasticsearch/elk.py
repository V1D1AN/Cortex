...
class ElasticsearchAnalyzer(Analyzer):
    ...
    def run(self):
        ...
        try:
            ...
            if len(fields) > 1:
                res = self.client.search(index=self.index, size=1000,request_timeout=30,body={"query": {"constant_score": {"filter": {"bool":{"should": should}}}}})
            else:
    ...
    def summary(self, raw):
        ...
        if len(raw["results"]) == 0:
            ...
            value = "{} events".format(len(raw["results"]))
            level = "suspicious"
            taxonomies.append(self.build_taxonomy(level, namespace, predicate, value))
