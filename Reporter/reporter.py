...
class Reporter(Responder):
    ...
    def getTLP(self,tlp):
        ...
        # Summary Fields - TLP
        if(tlp == 0):
            tlp = ['[TLP:WHITE]', '**TLP:WHITE - Disclosure is not limited.**']
        elif(tlp == 1):
            tlp = ['[TLP:GREEN]', '**TLP:GREEN - Limited disclosure, restricted to the community.**']
        elif(tlp == 2):
            tlp = ['[TLP:AMBER]', '**TLP:AMBER - Limited disclosure, restricted to participants’ organizations.**']
        elif(tlp == 3):
            tlp = ['[TLP:RED]', '**TLP:RED - Not for disclosure, restricted to participants only.**']
        else:
            tlp = "unknown"
